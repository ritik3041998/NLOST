...
def test_on_align_fk(model, n_iter, logWriter, params):
    ...
    test_model = nlost.NLOST(ch_in=1, num_coders=1,spatial=128,tlen=256,bin_len=0.0096,target_size=params.target_size)
    ...
    for i in range(len(all_file)): 
        ...
        if utils.is_main_process():
            with torch.no_grad():
                ...
                cv2.imwrite(out_path + f'n_iterdep{n_iter}_{i}.png', front_dep*255)
...
def test_on_align_xu(model, n_iter, logWriter, params):
    ...
    test_model = torch.nn.DataParallel(test_model,[0])
    test_model.load_state_dict(model_dict)
    rw_path  = params.nlost_datapath

    out_path = params.model_dir + '/test_on_nlost/'
    if not os.path.exists(out_path):
            os.makedirs(out_path, exist_ok=True) 
